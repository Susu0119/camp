...
const getUserRoleLabel = (roleCode) => {
  switch (roleCode) {
    case 1:
      return <span className="text-blue-400">일반 사용자</span>;
    case 2:
      return <span className="text-green-500">캠핑장 관계자</span>;
    case 3:
      return <span className="text-purple-500">관리자</span>;
    default:
...
export default function AdminUserList() {
  ...
  try {
    ...
    const res = await axios.get("/web/admin/users/search", { params });
    const data = Array.isArray(res.data) ? res.data : res.data.users || [];

    setUsers(data);             // 전체 목록 갱신
    setFilteredUsers(data);     // 필터링 목록 갱신
    setCurrentPage(1);
  } catch (err) {
  ...
  try {
    const res = await axios.get("/web/admin/users", {
    ...
    console.error("❌ 사용자 목록 초기화 실패:", err);
  }
};

  // 행 클릭스 상세 정보 조회 + 모달 열기
  const handleRowClick = async (providerCode, providerUserId) => {
    try {
      const res = await axios.get("/web/admin/users/detail", {
        params: { providerCode, providerUserId },
      });
      ...
      <main className="flex-1 px-8 py-6 max-w-screen-2xl mx-auto">
        ...
        <form onSubmit={handleSearch} className="mb-6 p-4 text-black/70 border border-gray-200 shadow-sm rounded-xl bg-white flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-4">
            <input type="text" placeholder="이름 or 이메일" value={keyword} onChange={(e) => setKeyword(e.target.value)} className="bg-purple-300/30 px-4 py-1 rounded-xl w-60 focus:outline-none shadow-sm" />
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                paginated.map((user, i) => (
                  <tr key={i} 
                  onClick={() => handleRowClick(user.providerCode, user.providerUserId)} 
                  className="hover:bg-purple-100 transition cursor-pointer">
        ...
        <div className="flex justify-center mt-6 gap-2 text-lg">
          ...
          {[...Array(totalPages).keys()].map((i) => (
            <button key={i} onClick={() => setCurrentPage(i + 1)} className={`h-9 w-9 flex items-center justify-center rounded-full cursor-pointer transition text-purple-900/70 ${currentPage === i + 1 ? 'bg-purple-100 text-purple-900/70' : 'hover:bg-purple-100 hover:shadow-sm'}`}>
              {i + 1}
            </button>
          ))}
        ...
        {modalOpen && (
          <AdminUserModal
            ...
            refreshList={() => {
              axios.get("/web/admin/users").then((res) => {
                const data = Array.isArray(res.data) ? res.data : res.data.users || [];
                setUsers(data);
                setFilteredUsers(data);
              });
